...
if ( ! function_exists( 'siteorigin_unwind_post_meta' ) ) {
	...
	function siteorigin_unwind_post_meta( $cats = true, $post_id = '' ) {
		...
		<?php
		if ( is_sticky() && is_home() && ! is_paged() && ! siteorigin_setting( 'blog_archive_layout' ) == 'grid' ) {
		...
		<?php
		if ( $comments && siteorigin_setting( 'blog_display_comments' ) ) {
...
if ( ! function_exists( 'siteorigin_unwind_thumbnail_meta' ) ) {
	...
	function siteorigin_unwind_thumbnail_meta() {
		if ( ! ( is_sticky() || siteorigin_setting( 'blog_display_category' ) ) ) {
			return;
		}
	...
	function siteorigin_unwind_archive_post_media() {
		if (
			(
				...
				siteorigin_unwind_get_gallery()
			) ||
			(
				get_post_format() == 'image' &&
				siteorigin_unwind_get_image()
			) ||
			(
				get_post_format() == 'video' &&
				siteorigin_unwind_get_video()
			) ||
			has_post_thumbnail()
		) {
